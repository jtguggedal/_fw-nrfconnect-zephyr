...
void main(void)
{
	...
	printk("BH1749 sample application\n");
	dev = device_get_binding(DT_ROHM_BH1749_0_LABEL);
	if (!dev) {
	...
	while (true) {
		...
		if (sensor_sample_fetch(dev)) {
			printk("Could not fetch sample\n");
		}

		sensor_channel_get(dev, SENSOR_CHAN_RED, &intensity);
